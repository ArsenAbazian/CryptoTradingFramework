...
namespace CryptoMarketClient.Bittrex {
    public class BittrexExchange : Exchange {
        ...
        public BittrexExchange() {
            Balances = new BittrexAccountBalanceInfoList();
        }

        public override string Name => "Bittrex";
        public BittrexCurrencyInfoList Currencies { get; } = new BittrexCurrencyInfoList();
        public new BittrexAccountBalanceInfoList Balances { get { return (BittrexAccountBalanceInfoList)base.Balances; } protected set { base.Balances = value; } }
        ...
        public override bool UpdateCurrencies() {
            ...
            foreach (JObject obj in markets) {
                ...
                BittrexCurrencyInfo c = Currencies[currency];
                if (c == null) {
                    c = new BittrexCurrencyInfo();
                    c.CurrencyTicker = currency;
                    c.CurrencyName = obj.Value<string>("CurrencyLong");
        ...
        public override bool GetCurrenciesInfo() {
            ...
            lock (Currencies) {
                ...
                foreach (JObject obj in markets) {
                    ...
                    c.CurrencyTicker = obj.Value<string>("Currency");
                    c.CurrencyName = obj.Value<string>("CurrencyLong");
                    c.MinConfirmation = obj.Value<int>("MinConfirmation");
                    c.TxFree = obj.Value<double>("TxFee");
                    c.IsActive = obj.Value<bool>("IsActive");
        ...
        public bool GetBalance(string currency) {
            ...
            try {
                ...
                JObject jsonBalance = res.Value<JObject>("result");
                return ProcessBalance(jsonBalance);
            } catch (Exception) {
                return false;
            }
        }
        ...
        public override bool UpdateBalances() {
            ...
            string address = string.Format("https://bittrex.com/api/v1.1/account/getbalances?apikey={0}&nonce={1}", Uri.EscapeDataString(ApiKey), DateTime.Now.ToUnixTime());
            WebClient client = GetWebClient();
            client.Headers.Clear();
            client.Headers.Add("apisign", GetSign(address));
            try {
                string text = client.DownloadString(address);
                if (!ProcessBalances(text))
                    return false;
            } catch {
                return false;
            }
        ...
        bool ProcessBalance(JObject jsonBalance) {
            string currency = jsonBalance.Value<string>("Currency");
            double balance = jsonBalance.Value<double>("Balance");
            lock (Balances) {
                BittrexAccountBalanceInfo info = Balances[currency];
                if (info == null) {
                    if (balance <= 0.00000000)
                        return true;
                    info = new BittrexAccountBalanceInfo(currency);
                    Balances.Add(info);
                } else if (balance <= 0.00000000) {
                    Balances.Remove(info);
                    return true;
                }
                info.CurrencyName = Currencies[info.CurrencyTicker].CurrencyName;
                info.Balance = balance;
                info.Available = jsonBalance.Value<double>("Available");
                info.Pending = jsonBalance.Value<double>("Pending");
            }
            RaiseBalancesChanged();
            return true;
        }

        bool ProcessBalances(string text) {
            if (string.IsNullOrEmpty(text))
                return false;
            JObject res = (JObject)JsonConvert.DeserializeObject(text);
            if (res.Value<bool>("success") == false) {
            ...
            JArray jsonBalances = res.Value<JArray>("result");
            foreach (JObject jsonBalance in jsonBalances) {
                if (!ProcessBalance(jsonBalance))
                    return false;
            }
            return true;
        }
        ...
        string OnGetDeposit(string currency, string text) {
            ...
            BittrexAccountBalanceInfo info = (BittrexAccountBalanceInfo)Balances.FirstOrDefault(b => b.CurrencyTicker == currency);
            info.CurrencyTicker = addr.Value<string>("Address");
            return info.CurrencyTicker;
        }
    }
}
