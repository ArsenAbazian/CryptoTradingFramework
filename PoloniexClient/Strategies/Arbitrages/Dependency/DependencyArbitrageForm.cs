...
namespace CryptoMarketClient {
    public partial class DependencyArbitrageForm : XtraForm {
        public DependencyArbitrageForm() {
            ...
            gridControl1.DataSource = new System.ComponentModel.BindingList<Crypto.Core.Common.Arbitrages.DependencyArbitrageInfo>() /* TODO: Assign the real data here.*/;
        ...
        private void biAdd_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            using(DependencyArbitrageEditingForm form = new DependencyArbitrageEditingForm()) {
                form.Add();
                if(form.ShowDialog() != DialogResult.OK)
                    return;
                if(ArbitrageHelper.IsActive)
                    form.ArbitrageInfo.StartListenOrderBookStreams();
                ArbitrageHelper.Add(form.ArbitrageInfo);
        ...
        private void biEdit_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            using(DependencyArbitrageEditingForm form = new DependencyArbitrageEditingForm()) {
                form.Edit((DependencyArbitrageInfo)this.gridView1.GetFocusedRow());
                form.ArbitrageInfo.StopListenOrderBookStreams();
                if(form.ShowDialog() != DialogResult.OK)
                    return;
                ArbitrageHelper.Save();
                if(ArbitrageHelper.IsActive)
        ...
        private void biRemove_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            if(XtraMessageBox.Show("Dou you really want to remove selected items?", "Removing", MessageBoxButtons.YesNoCancel) != DialogResult.Yes)
                return;
            List<DependencyArbitrageInfo> selectedItems = ArbitrageHelper.Items.Where(i => i.IsSelectedInDependencyArbitrageForm).ToList();
            foreach(DependencyArbitrageInfo item in selectedItems) {
                if(ArbitrageHelper.IsActive)
        ...
        private void biShowChart_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            DependencyArbitrageInfo info = (DependencyArbitrageInfo)this.gridView1.GetFocusedRow();
        ...
        private void biAllHistoryChart_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            ...
            DependencyArbitrageInfo all = new DependencyArbitrageInfo();
            all.Assign(info);
            all.LoadHistory();
